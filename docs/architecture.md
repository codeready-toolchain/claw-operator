...
**Wire format API**: Each provider in `knownProviders` declares the OpenClaw wire format API it requires (e.g., `google-generative-ai` for Google, `anthropic-messages` for Anthropic, `openai-responses` for xAI, `openai-chatgpt-responses` for the internal openai-codex companion). `buildProviderEntry()` bakes the correct `api` field into every `models.providers` entry at generation time. Providers using the OpenClaw default (`openai-completions`) — like `openai` and unknown providers — omit the field entirely.
