...
**Decision:** Option B (revised after implementation) — replace nginx with a purpose-built Go **hybrid credential-injecting proxy** built on `github.com/elazarl/goproxy`, operating in two modes:

1. **Gateway mode (API gateway):** For LLM provider calls where the credential has `provider` set on `spec.credentials`. OpenClaw is configured with `baseUrl` pointing to the proxy (e.g., `http://claw-proxy:8080/gemini/v1beta`). The proxy strips the path prefix, injects credentials, and forwards to the upstream LLM API over HTTPS. This avoids the Google ADK JS SDK `thought_signature` bug that occurs with `env-proxy` mode on streamed responses.

2. **MITM forward proxy mode:** For general egress traffic (web_fetch tools, npm, OTEL, etc.). OpenClaw uses `HTTP_PROXY`/`HTTPS_PROXY` env vars. The proxy intercepts CONNECT tunnels, terminates TLS with an operator-managed CA, injects credentials for allowed domains, and forwards as HTTPS.

Config-driven routing, pluggable injectors (bearer, API key header, GCP ADC/OAuth2 with token vending), explicit auth header stripping on all requests (defense in depth — strip before inject, not just overwrite). The `provider` field on `spec.credentials` determines which mode is used and triggers dynamic generation of the `models.providers` section in `openclaw.json`. Add health endpoint, strict header policy, credential redaction in logs, and 502 on injection failure.

The proxy derives its routing configuration from `spec.credentials` (Q2) — the operator reads the credential entries and generates both the proxy's config JSON and the OpenClaw `openclaw.json` dynamically. Adding a new LLM provider is a new entry in `spec.credentials` with `provider` set — no manual config editing. See [Q12](security-hardening-impl-questions.md) for the full traffic routing decision and rationale.

**Why hybrid (revised from pure MITM):** The original decision called for pure MITM. During implementation, we discovered the Google ADK JS SDK strips the `thought_signature` field from function call parts in `env-proxy` mode with streaming, causing 400 errors from Gemini after tool use. Gateway mode (`baseUrl`) bypasses this SDK code path entirely. MITM is retained for general egress where the SDK bug doesn't apply. The MITM CA is ~30 LOC of Go (`crypto/x509`) and follows the same Secret management pattern the operator already uses. On OpenShift, `config.openshift.io/inject-trusted-cabundle` handles corporate proxy CAs for the outbound leg.

_Considered and rejected: Option A — nginx + sidecar for OAuth (band-aid, doesn't fix header stripping or static config), Option D — keep nginx and defer (blocks Vertex AI, accumulates technical debt), pure MITM (Option B original — blocked by Google ADK JS SDK bug on streamed tool responses)._
