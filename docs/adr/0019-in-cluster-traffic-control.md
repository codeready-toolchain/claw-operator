...
| Q3 | In-cluster MCP servers | Add `credentialRef` to `McpServerSpec` for proxy-injected auth; passthrough for unauthenticated | Gateway never sees raw MCP tokens (same security model as LLM providers); follows established `CustomProviderSpec.credentialRef` pattern; works for both in-cluster and external MCP servers |
