...
- [x] 1.1 Add `OpenClawGatewaySecretName = "openclaw-gateway-token"` constant to reconciler
- [x] 1.2 Add `GatewayTokenKeyName = "token"` constant to reconciler
...
- [x] 3.2 Check if `openclaw-gateway-token` secret already exists in the namespace
- [x] 3.3 If secret exists and has `token` entry, skip token generation
- [x] 3.4 If secret doesn't exist, generate new token using `generateGatewayToken()`
- [x] 3.5 Create Secret object with `token` data entry
...
- [x] 5.3 Add test: Secret contains `token` entry with 64 hex characters
...
- [x] 6.1 Update CLAUDE.md to document `openclaw-gateway-token` Secret and its purpose
