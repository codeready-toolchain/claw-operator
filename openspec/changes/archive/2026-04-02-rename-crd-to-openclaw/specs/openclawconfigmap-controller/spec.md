...
The system SHALL implement the Reconcile function to fetch the OpenClaw resource and create a ConfigMap using the manifest from `internal/manifests/configmap.yaml`.
...
- **WHEN** the OpenClaw exists and no ConfigMap named 'openclaw-config' exists
...
- **WHEN** the OpenClaw exists and a ConfigMap named 'openclaw-config' already exists
...
- **THEN** the controller SHALL set the OpenClaw as the controller owner reference on the ConfigMap
...
The system SHALL generate RBAC markers and permissions for the controller to manage OpenClaw and ConfigMap resources.
...
The system SHALL configure the controller to watch ConfigMap resources that are owned by OpenClaw resources.

#### Scenario: Controller triggers on ConfigMap create
- **WHEN** a ConfigMap with an owner reference to an OpenClaw is created
- **THEN** the controller's Reconcile function SHALL be invoked

#### Scenario: Controller triggers on ConfigMap update
- **WHEN** a ConfigMap with an owner reference to an OpenClaw is updated
- **THEN** the controller's Reconcile function SHALL be invoked

#### Scenario: Controller triggers on ConfigMap delete
- **WHEN** a ConfigMap with an owner reference to an OpenClaw is deleted
- **THEN** the controller's Reconcile function SHALL be invoked
