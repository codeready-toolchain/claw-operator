...
The OpenClaw operator currently manages a single `Claw` CRD for OpenClaw instances. This change introduces a second CRD (`NodePairingRequest`) to handle node pairing operations. The operator uses Kubebuilder/Operator SDK framework with controller-runtime for reconciliation patterns.
...
**Goals:**
- Add NodePairingRequest CRD with namespaced scope
- Implement minimal controller to watch and reconcile NodePairingRequest resources
...
**Rationale:** The project uses envtest with testify assertions, shared `TestMain` setup in `suite_test.go`, and separate test files per concern. Reusing these patterns ensures consistency and leverages existing test infrastructure.
...
### Decision 4: Spec.RequestID as required string field
**Rationale:** RequestID serves as the unique identifier for pairing operations. Making it required (via kubebuilder validation marker) ensures callers provide this critical field at creation time.
...
1. Generate API types with kubebuilder markers (`api/v1alpha1/nodepairingrequest_types.go`)
...
5. Add RBAC markers to controller
6. Register controller in `cmd/main.go`
7. Write envtest-based controller tests
8. Run `make install` to install CRDs in development cluster
9. Verify CRD registration with `kubectl get crd nodepairingrequests.openclaw.sandbox.redhat.com`

**Rollback:** Remove CRD with `kubectl delete crd nodepairingrequests.openclaw.sandbox.redhat.com` and revert code changes. No data migration needed (new feature).
...
- Do we need printcolumns for `kubectl get nodepairingrequest` output beyond name and age?
