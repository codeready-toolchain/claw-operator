...
The OpenClaw status URL currently provides the Route HTTPS endpoint but requires users to manually copy the gateway token from the `openclaw-secrets` Secret and paste it into the UI. This creates unnecessary friction in the user experience and requires additional kubectl commands to retrieve the token.
...
- Update the `updateStatus()` method in the controller to read the gateway token from the `openclaw-secrets` Secret and append it as a URL fragment
- Ensure the token value is retrieved from the `OPENCLAW_GATEWAY_TOKEN` key in the `openclaw-secrets` Secret
