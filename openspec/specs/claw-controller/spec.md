...
- **THEN** the Claw controller SHALL be registered with the controller-runtime manager
...
The system SHALL configure the controller to watch for create, update, and delete events on Claw resources.

#### Scenario: Controller triggers on create
- **WHEN** an Claw resource is created
- **THEN** the controller's Reconcile function SHALL be invoked

#### Scenario: Controller triggers on update
- **WHEN** an Claw resource is updated
- **THEN** the controller's Reconcile function SHALL be invoked

#### Scenario: Controller triggers on delete
- **WHEN** an Claw resource is deleted
- **THEN** the controller's Reconcile function SHALL be invoked

### Requirement: Controller reconciles all Claw resources
The system SHALL configure the controller to watch all Claw resources but only reconcile resources named "instance", skipping all other named resources.

#### Scenario: Resource named 'instance' triggers reconciliation
- **WHEN** an Claw named 'instance' is created
...
- **WHEN** an Claw with a name other than 'instance' is created
...
- **WHEN** multiple Claw resources exist with different names
...
- **WHEN** an Claw with a name other than 'instance' is reconciled
...
The system SHALL implement the Reconcile function to fetch the Claw resource and create resources in the following order: ConfigMap first using the manifest from `internal/manifests/configmap.yaml`, then Deployment using the manifest from `internal/manifests/deployment.yaml` only after the ConfigMap exists.

#### Scenario: Reconciliation fetches the Claw resource
- **WHEN** the Reconcile function is invoked
- **THEN** it SHALL fetch the Claw resource from the API server using the namespace and name from the reconcile request

#### Scenario: Reconciliation handles resource not found
- **WHEN** the Claw resource does not exist (e.g., deleted)
...
- **WHEN** the Claw exists and no ConfigMap exists for it
...
- **WHEN** the Claw exists and a ConfigMap already exists
...
- **WHEN** the Claw exists and ConfigMap 'claw-config' exists but no Deployment exists
...
- **WHEN** the Claw exists but ConfigMap 'claw-config' does not exist yet
...
- **WHEN** the Claw exists and a Deployment already exists
...
- **THEN** the controller SHALL set the Claw as the controller owner reference on the ConfigMap

#### Scenario: Reconciliation establishes ownership on Deployment
- **WHEN** creating the Deployment
- **THEN** the controller SHALL set the Claw as the controller owner reference on the Deployment

### Requirement: Controller has RBAC permissions
The system SHALL generate RBAC markers and permissions for the controller to watch and reconcile Claw resources.

#### Scenario: Controller can list Claw resources
- **WHEN** the controller starts
- **THEN** it SHALL have permissions to list Claw resources

#### Scenario: Controller can watch Claw resources
- **WHEN** the controller is running
- **THEN** it SHALL have permissions to watch Claw resources

#### Scenario: Controller can update Claw status
...
### Requirement: Deployment is created in same namespace as Claw
The system SHALL create the Deployment resource in the same namespace as the owning Claw resource.

#### Scenario: Namespace matches owner
- **WHEN** an Claw is created in namespace "test-ns"
- **THEN** the created Deployment SHALL also be in namespace "test-ns"

### Requirement: Deployment is deleted when Claw is deleted
The system SHALL configure owner references such that Kubernetes garbage collection automatically deletes the Deployment when the owning Claw is deleted.

#### Scenario: Garbage collection deletes Deployment
- **WHEN** an Claw is deleted
...
The system SHALL check the Claw resource name early in the reconciliation loop and skip processing if the name is not "instance".

#### Scenario: Name check occurs after fetch
- **WHEN** the Reconcile function fetches the Claw resource
...
The system SHALL configure the controller to watch ConfigMap resources that are owned by Claw resources.

#### Scenario: Controller triggers on ConfigMap create
- **WHEN** a ConfigMap with an owner reference to an Claw is created
- **THEN** the controller's Reconcile function SHALL be invoked

#### Scenario: Controller triggers on ConfigMap update
- **WHEN** a ConfigMap with an owner reference to an Claw is updated
- **THEN** the controller's Reconcile function SHALL be invoked

#### Scenario: Controller triggers on ConfigMap delete
- **WHEN** a ConfigMap with an owner reference to an Claw is deleted
- **THEN** the controller's Reconcile function SHALL be invoked
